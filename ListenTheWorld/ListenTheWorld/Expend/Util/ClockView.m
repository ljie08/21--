...
- (void)initialization{
    ...
    imageView =[[ UIImageView alloc] initWithImage:IMGNAMED(@"star")];
...
- (void)setSliderModel:(kClockViewModel)sliderModel{
    _sliderModel = sliderModel;
    CGFloat itemWidth = 170 + 20 + 50;//150

    CGFloat maxWidth = self.frame.size.width;///<.frame.size.height?self.frame.size.width:self.frame.size.height;
    CGPoint center = CGPointMake(maxWidth/2.0, self.height/2.0);
    //    CGFloat radius = maxWidth/2.0-_strokeWidth/2.0-1;//留出一像素，防止与边界相切的地方被切平
    CGFloat radius = itemWidth/2.0;//maxWidth/2.0;//maxWidth/2.0-_strokeWidth/2.0 - 1 - 3;//留出一像素，防止与边界相切的地方被切平
    ...
    for (NSInteger i = 0; i <12; i++) {
        ...
        NSInteger m = (i<= 6 && i >= 0) ? 1 : 0;
        CGFloat width = 6;
        CGFloat height = 2;
        CGFloat originX = center.x + (radius * cos(sAngel) + width * m * (-1))*l;
        CGFloat originY = center.y + (radius * sin(sAngel) + height * m  * (-1))*l;
        
        CGPoint orign = CGPointMake(originX,originY);
        ScaleView *scaleView = [[ScaleView alloc] initWithFrame:CGRectMake(0, 0,width, 2)];
        scaleView.origin = orign;
//        scaleView.center= orign;
        scaleView.bgColor = [UIColor whiteColor];
        scaleView.roateAngel = ToDeg(sAngel);
        scaleView.tag = 1000 + i;
        [self addSubview:scaleView];
        sAngel += M_PI_2/3.0 ;
    }
    
//    if (sliderModel == kClockViewScale) {
//        CGFloat space = 0;
//        NSArray *arr = [NSArray arrayWithObjects:@"0",@"15",@"30",@"45", nil];
//        for (NSInteger i = 0; i<40; i++) {
//            UIImage *img =  [UIImage imageNamed:@"autoDot"];//IMGNAME(@"autoDot");
//            if (i % 10 == 0) {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint center = CGPointMake(maxWidth/2.0, self.frame.size.height/2.0);
    //    CGFloat radius = maxWidth/2.0-_strokeWidth/2.0-1;//留出一像素，防止与边界相切的地方被切平
    CGFloat itemWidth = 170 + 20 + 50;//150
    CGFloat radius = (itemWidth - 20)/2.0-_strokeWidth/2.0;//maxWidth/2.0-_strokeWidth/2.0 - 1 - 3;//留出一像素，防止与边界相切的地方被切平
    ...
    basePath.lineCapStyle = kCGLineCapRound;
    basePath.lineJoinStyle = kCGLineJoinRound;
    [_pathBackColor set];
    [basePath stroke];
    
    //虚圆 背景线
    CGFloat lengths1[] = {2,2};
    
    CGContextSetLineDash(context, 0, lengths1, 2);
    CGContextSetLineWidth(context, _strokeWidth);
    CGContextAddEllipseInRect(context, CGRectMake((maxWidth -(itemWidth - 20 - 20))/2.0, (self.height -(itemWidth - 20 - 20))/2.0, (itemWidth - 20 - 20), (itemWidth - 20 - 20))); //椭圆
...
- (void)backToStartPoint{
    ...
    [lastScaleView actionRotateAnimationClockwise:NO];
}

//- (void)setStarRotate:(CGFloat)rotate{
//    imageView.hidden = NO;
//    CGFloat maxWidth = self.frame.size.width;
//    CGPoint center = CGPointMake(maxWidth/2.0, self.frame.size.height/2.0);
//    CGFloat itemWidth = 170 + 20 + 50;//150
//    CGFloat radius = (itemWidth + 20)/2.0;//maxWidth/2.0-_strokeWidth/2.0 - 1 - 3;//留出一像素，防止与边界相切的地方被切平
//
//    CGPoint point = [self calculateTextPositionWithCenter:center Angle:rotate Radius:radius];
...
//    [self setStarRotate:M_PI * 2 * progress - M_PI_2];
    
    NSInteger tag = progress * 60;
    if (tag %5 == 0) {
        
        for (UIView *view in self.subviews) {
            if (view.tag == 1000 + tag/5) {
                ScaleView *sView = (ScaleView *)view;
                [sView actionRotateAnimationClockwise:YES];
                lastScaleView = sView;
                [self performSelector:@selector(backToStartPoint) withObject:nil afterDelay:0.5];
            }
        }
    ...
    _timer = [NSTimer scheduledTimerWithTimeInterval:0.005 repeats:YES block:^(NSTimer * _Nonnull timer) {
        ...
        //数值增加或者减少
        if (_animationModel == kClockViewIncreaseSameTime) {
...
//    
//    if (_sliderModel == kClockViewProgress) {
//        CGFloat maxWidth = self.frame.size.width<self.frame.size.height?self.frame.size.width:self.frame.size.height;
//        CGFloat radius = maxWidth/2.0-_strokeWidth/2.0-1;//留出一像素，防止与边界相切的地方被切平
...
- (void)initLayers{
    ...
    trackLayer. strokeColor = [_pathFillColor  CGColor];//边框颜色
    
    trackLayer. opacity =  1;
    
    trackLayer.lineCap  = kCALineCapRound ;
    
    trackLayer. lineWidth = _strokeWidth * 1.5;  // 红色的边框宽度
...
- (void)addPath:(CGFloat)angle{
    ...
    CGPoint center = CGPointMake(maxWidth/2.0, self.height/2.0);
    CGFloat itemWidth = 170 + 20 + 50;//150
    CGFloat radius = (itemWidth - 20 - 20)/2.0-_strokeWidth/2.0;//(maxWidth - 20 - 20)/2.0-_strokeWidth/2.0;//maxWidth/2.0-_strokeWidth/2.0 - 1;//留出一像素，防止与边界相切的地方被切平
    CGFloat valueEndA;
    if (angle != 0) {
